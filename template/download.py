...
if __name__ == '__main__':
    ...
    temporalFilter = {'startDate': '2000-12-10', 'endDate': '2000-12-10'}
    ...
    datasets = sendRequest(serviceUrl + "dataset-search",
                           datasetSearchParameters, apiKey)
    ...
    for dataset in datasets:
        ...
        if dataset['datasetAlias'] != datasetName:
            print("Found dataset " +
                  dataset['collectionName'] + " but skipping it.\n")
            continue

        # I don't want to limit my results, but using the dataset-filters request, you can
        # find additional filters

        sceneSearchParameters = {'datasetName': dataset['datasetAlias'],
                                 'maxResults': 2,
                                 'startingNumber': 1,
                                 'sceneFilter': {'ingestFilter': {'start': '2000-05-28'},
                                                 'spatialFilter': spatialFilter,
                                                 'temporalFilter': temporalFilter}}
        ...
        if scenes['recordsReturned'] > 0:
            ...
            downloadOptions = sendRequest(
                serviceUrl + "download-options", downloadOptionsParameters, apiKey)
            ...
            if downloads:
                ...
                requestResults = sendRequest(
                    serviceUrl + "download-request", downloadRequestParameters, apiKey)
                ...
                if requestResults['preparingDownloads'] != None and len(requestResults['preparingDownloads']) > 0:
                    ...
                    moreDownloadUrls = sendRequest(
                        serviceUrl + "download-retrieve", downloadRetrieveParameters, apiKey)
                    ...
                    while len(downloadIds) < requestedDownloadsCount:
                        preparingDownloads = requestedDownloadsCount - \
                            len(downloadIds)
                        print("\n", preparingDownloads,
